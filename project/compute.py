...
def distribute_weight(session, service_name, api_name, weight, depth):
	...
	session.run(cypher_query + 'return downstream', parameters={'sname':service_name,'aname':api_name,'w':weight})
	
#distribute weights from multiple nodes

import read

def distribute_multiple_weights(session, starting_nodes, depth):
	...
	for starting_node in starting_nodes:
		...
		for api_name in starting_node['api_names']:
			...
			message = distribute_weight(session, service_name, api_name, hit_rate, depth)
			if message != None:
